...
`.` 会被 _优先_ 识别为数字常量的一部分，然后才是对象属性访问运算符
于是：
...
基本类型值没有 `.length`, `.toString()` 这种属性和方法，需要通过封装对象才能访问，所以 js 会 _自动为基本类型值包装一个对象_
...
** 在需要用到封装对象中的基本类型值的地方会发生 _隐式拆封_ **

# 3 coercion type casting

js 中强制类型转换总是返回标量基本类型值，不会返回对象和函数
