...
2. 浮动元素之后的元素设置 `clear` 以闭合相关方向的浮动时，根据w3c规范规定，闭合浮动的元素会在其 `margin-top` 以上产生一定的空隙（`clearance`），该空隙会阻止元素 `margin-top` 的折叠，并作为间距存在于元素的 `margin-top` 的上方
3. `inline-block` 元素与其兄弟元素、子元素和父元素的外边距都不会折叠（包括其父元素和子元素）
